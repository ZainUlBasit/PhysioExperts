...
import { AddNewCategoryApi } from "../../Api_Requests/Api_Requests";
import { fetchCategories } from "../../store/Slices/CategorySlice";

const AddNewCategoryModal = ({ Open, setOpen }) => {
  ...
  const handleAddCategory = async () => {
    setLoading(true);
    try {
      const response = await AddNewCategoryApi({ name });
      if (response.data.success) {
        SuccessToast("Category added successfully!");
        dispatch(fetchCategories());
        setOpen(false);
      } else {
        ErrorToast("Unable to add category");
      }
    } catch (err) {
      console.log(err);
