...
const EditExerciseModal = ({ Open, setOpen, exerciseData }) => {
  const [name, setName] = useState(exerciseData.title || "");
  const [selectedFile, setSelectedFile] = useState(null);
  const [SelectedCategory, setSelectedCategory] = useState(
    exerciseData.categoryId._id || ""
  );
  const [SelectedCategoryName, setSelectedCategoryName] = useState(
    exerciseData.categoryId.name || ""
  );
  const CategoryState = useSelector((state) => state.CategoryState);
  const [existingImageUrl, setExistingImageUrl] = useState(
    exerciseData.sourceUrl || ""
  );
  ...
  const handleUpdateExercise = async () => {
    ...
    try {
      ...
      const response = await UpdateExerciseApi(exerciseData._id, {
        title: name,
        sourceUrl: imageUrl,
      ...
      if (response.data.success) {
        ...
        ErrorToast("Unable to update exercise");
      }
    } catch (err) {
      console.log(err);
