          ...
          <div className="flex w-full border-2 border-custom-bg rounded-lg">
                  ...
                  ContactsState.data.map((dt) => {
                    return AuthState.data.role === 2 ? (
                      <div
                        ...
                        onClick={() => {
                          setSelectedId(dt.patientId._id);
                        }}
                      >
                        <img
                          className="w-[70px] rounded-full h-[70px] object-cover border-2 border-custom-bg"
                          src={dt.patientId.imageUrl}
                          alt=""
                        />
                        <div className="font-bold">{dt.patientId.name}</div>
                      </div>
                    ) : (
                      <div
                        key={dt.doctorId._id}
            ...
            <div className="w-full">
              ...
              <div className="bg-white flex flex-col h-[70vh] rounded-lg overflow-y-auto border-l-2 border-l-custom-bg">
                {DoctorState.data
                  .find((dt) => dt._id === AuthState.data.doctorId._id)
                  ?.Chats.filter((dt) => {
                    return dt.patientId === SelectedId;
                  })
                  .map((d) => {
                    return (
                      <div
                  ...
                  onClick={async () => {
                    try {
                      const response = await SendMessageAPI({
                        ...
                        doctorId: AuthState.data.doctorId._id,
                        message: Message,
